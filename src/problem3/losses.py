...
def _entropy(probability: torch.Tensor) -> torch.Tensor:
    return -(probability.clamp_min(1e-8) * probability.clamp_min(1e-8).log()).sum(dim=-1).mean()


def supervised_loss(
    outputs: dict[str, torch.Tensor], batch: dict[str, Any], class_weights: torch.Tensor,
    cfg: dict[str, Any],
    ...
    correlation = _correlation_loss(outputs["regression"], labels_reg)
    ccc = _ccc_loss(outputs["regression"], labels_reg)
    ...
    aux_cls = torch.stack([
        F.cross_entropy(outputs["aux_logits"][:, index], labels_cls, weight=class_weights,
                        label_smoothing=float(cfg["label_smoothing"])) for index in range(3)
    ]).mean()
    ...
    parts = {"classification": classification, "regression": regression, "correlation": correlation,
             "ccc": ccc, "coherence": coherence, "aux_classification": aux_cls,
             "aux_regression": aux_reg, "temporal_sparsity": temporal, "modality_sparsity": modality}
