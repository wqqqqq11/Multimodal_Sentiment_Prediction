...
def predict(model: torch.nn.Module, loader: DataLoader[dict[str, Any]], device: torch.device, *,
            goals: dict[str, float] | None = None, calibration: dict[str, Any] | None = None) -> dict[str, Any]:
    model.eval()
    sample_ids, raw_texts, logits, regression, log_variance, gates = [], [], [], [], [], []
    attentions = {name: [] for name in ("text", "audio", "vision")}
    labels_cls, labels_reg = [], []
    for source in loader:
        sample_ids.extend(source["sample_id"])
        raw_texts.extend(source["raw_text"])
        ...
        for name in attentions:
            attentions[name].append(outputs[f"time_attention_{name}"].float().cpu().numpy())
    ...
    raw_logits, raw_regression = np.concatenate(logits), np.concatenate(regression)
    probabilities, prediction_class, prediction_regression = apply_calibration(raw_logits, raw_regression, calibration)
    result: dict[str, Any] = {
        "sample_id": np.asarray(sample_ids), "raw_text": np.asarray(raw_texts), "logits": raw_logits,
        "probabilities": probabilities, "prediction_class": prediction_class,
        "prediction_regression": prediction_regression, "prediction_regression_raw": raw_regression,
    ...
    if labels_cls:
        result["label_class"], result["label_regression"] = np.concatenate(labels_cls), np.concatenate(labels_reg)
        result["metrics"] = all_metrics(result["label_class"], prediction_class,
                                        result["label_regression"], prediction_regression, goals)
    return result
...
def prediction_frame(result: dict[str, Any], include_labels: bool = True) -> pd.DataFrame:
    ...
    frame = pd.DataFrame({
        "sample_id": result["sample_id"], "raw_text": result["raw_text"],
        "predicted_polarity": result["prediction_class"].astype(int),
        "predicted_label": [names[i] for i in result["prediction_class"].astype(int)],
        "predicted_intensity": result["prediction_regression"],
        "prob_negative": result["probabilities"][:, 0], "prob_neutral": result["probabilities"][:, 1],
        ...
        "gate_text": result["modality_gate"][:, 0], "gate_audio": result["modality_gate"][:, 1],
        "gate_vision": result["modality_gate"][:, 2],
    })
    if include_labels and "label_class" in result:
        frame["true_polarity"], frame["true_intensity"] = result["label_class"].astype(int), result["label_regression"]
