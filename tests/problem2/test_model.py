...
def test_model_forward_uses_preprocessed_interface() -> None:
    ...
    assert outputs["classification_gates"].shape == (4, 3)
    assert outputs["regression_gates"].shape == (4, 3)
    ...
    assert torch.allclose(outputs["classification_gates"].sum(dim=1), torch.ones(4), atol=1e-5)
    assert torch.allclose(outputs["regression_gates"].sum(dim=1), torch.ones(4), atol=1e-5)
    assert torch.isfinite(outputs["regression"]).all()
    model.ablation_mode = "text_only"
    text_only = model(batch)
    assert torch.allclose(text_only["logits"], text_only["text_logits"], atol=1e-6)
    assert torch.allclose(text_only["regression"], text_only["text_regression"], atol=1e-6)
